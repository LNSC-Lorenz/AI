...
<#
.SYNOPSIS
    Create cloud-init cidata ISO for Ubuntu Autoinstall (RPA Platform)
.DESCRIPTION
...
.EXAMPLE
    ...
    Create cidata-<hostname>.iso in script directory (hostname read from user-data)
.EXAMPLE
...
if ([string]::IsNullOrEmpty($OutputPath)) {
    # Name the ISO after the target server's hostname (from user-data autoinstall config)
    $HostName = "rpa"
    $userDataPath = Join-Path $SourceDir "user-data"
    if (Test-Path -LiteralPath $userDataPath) {
        $m = Select-String -LiteralPath $userDataPath -Pattern '^\s*hostname:\s*(\S+)' | Select-Object -First 1
        if ($m) { $HostName = $m.Matches[0].Groups[1].Value }
    }
    $OutputPath = Join-Path $PSScriptRoot "cidata-$HostName.iso"
}
...
function New-IsoWithOscdimg {
    ...
    # Build argument string (oscdimg needs quoted paths for special characters)
...
Write-Host "  For Ubuntu 24.04 LTS Autoinstall (RPA Platform)" -ForegroundColor Blue
Write-Host "=============================================================" -ForegroundColor Blue

$resolved = Resolve-Path -LiteralPath $SourceDir -ErrorAction SilentlyContinue
if (-not $resolved -or -not (Test-Path -LiteralPath $resolved.Path)) {
    ...
    switch ($choice) {
        "1" {
            ...
            try {
                ...
                Write-Host "[Error] Failed to download or install ADK: $_" -ForegroundColor Red
                Write-Host "`nPlease try manual installation:" -ForegroundColor Yellow
                Write-Host "  1. Download from: https://aka.ms/adk" -ForegroundColor Yellow
                Write-Host "  2. Run adksetup.exe and select 'Deployment Tools'" -ForegroundColor Yellow
                exit 1
            }
        }
        "2" {
...
--------------------------
1. Download Windows ADK from: https://aka.ms/adk
2. Run the installer (adksetup.exe)
3. Select 'Deployment Tools' feature
...
Alternative Methods:
--------------------
A. Use cdrtools (mkisofs):
   Download: https://github.com/erinata/cdrtools
   Command: mkisofs -o cidata.iso -V cidata -J -R user-data meta-data
...
try {
    # Stage only required files to a temp directory (avoids including .ps1, verify.sh, etc.)
    ...
    Get-ChildItem -LiteralPath $stageDir | ForEach-Object { Write-Host "  $($_.Name) ($($_.Length) bytes)" -ForegroundColor Gray }
    ...
    Write-Host "  * user-data and meta-data must be lowercase, no extension" -ForegroundColor Green
