...
echo ""
echo "============================================="
echo "  Set magentic password (CIS policy: min 14 chars,"
echo "  must include uppercase, lowercase, digit, special)"
echo "  Example: ChangeMe2026!@#"
echo "============================================="
while true; do
    read -rsp "  New password for magentic: " NEW_PASS
    echo ""
...
echo "${NEW_PASS}" | passwd --stdin magentic 2>/dev/null || echo "magentic:${NEW_PASS}" | chpasswd
unset NEW_PASS CONFIRM_PASS
faillock --user magentic --reset 2>/dev/null || true
ok "magentic password updated. Use new password after reboot."
echo ""
...
# Ensure magentic is in sudo group (required by AllowGroups sudo)
usermod -aG sudo magentic
ok "magentic added to sudo group"

# Restrict SSH to sudo group only
sed -i '/^AllowGroups/d' "$SSHD_CONFIG"
echo "AllowGroups sudo" >> "$SSHD_CONFIG"
ok "SSH restricted to sudo group (magentic is member)"
...
# IP 转发禁用（CIS）- Docker 需要 ip_forward=1，此处跳过
# 注意: Docker 安装后会设置 ip_forward=1，这是 Docker 正常工作所必需的
# 如需禁用 IP 转发，请在 Docker 不使用桥接网络时执行
info "IP 转发由 Docker 管理（Docker 桥接网络需要 ip_forward=1）"
...
magentic hard nproc  65536
magentic soft fsize unlimited
magentic hard fsize unlimited
...
if systemctl is-active docker >/dev/null 2>&1; then
    ok "Docker daemon 已加固并启动成功 (overlay2, 无容器间通信, syslog 日志)"
else
    fail "Docker 启动失败，请检查 journalctl -u docker"
fi
...
echo "  [ ] Docker: $(systemctl is-active docker 2>/dev/null)" >> /var/log/cis-hardening-report.txt
echo "  [ ] Docker 加固: $(cat /etc/docker/daemon.json 2>/dev/null | head -1)" >> /var/log/cis-hardening-report.txt
