# Ubuntu 24.04 LTS Autoinstall 在 ESXi 上的无人值守部署 (Agentic-UI + Playwright)

## 概述

使用 Ubuntu 24.04 LTS Server 的 **Autoinstall**（Subiquity 自动安装）功能，在 VMware ESXi 上实现完全无人值守安装。
安装后手动执行安全加固和 Agentic-UI 部署脚本。

**本方案使用 Agentic-UI + Playwright**，替代 Magentic-UI + Quicksand VM：
- **更快**：无 VM 启动开销，无 CUA 视觉编码器瓶颈
- **更轻**：浏览器直接由 Playwright 控制
- **模型不变**：仍使用 qwen3:32b + qwen2.5vl-fast
...
│   LAB-Magentic-UI-02               │  HTTP   │   10.87.5.55                 │
│                                    │────────►│                              │
│  ┌──────────────────────────────┐  │         │  ┌────────────────────────┐  │
│  │  nginx (:8081)               │  │         │  │  Ollama (:11434)       │  │
│  │   └► Agentic-UI (:8082)      │  │         │  │  ├─ qwen3:32b (32B)    │  │
│  │       └► Playwright          │  │         │  │  ├─ qwen2.5vl-fast (8B)│  │
│  │          └► Chromium         │  │         │  │  KEEP_ALIVE=-1         │  │
│  └──────────────────────────────┘  │         │  └────────────────────────┘  │
│  ┌──────────────────────────────┐  │         │                              │
│  │  Docker + UFW + Fail2ban     │  │         └──────────────────────────────┘
│  └──────────────────────────────┘  │
...
**模型分工**：
- `qwen3:32b`（编排器）：规划任务，拆分为单步浏览器目标
- `qwen2.5vl-fast`（浏览器代理）：根据截图输出下一步动作（JSON 格式）
...
| `deploy-agentic-ui.sh` | Agentic-UI + Playwright 部署脚本（连接远程 Ollama） |
...
| 项目 | 值 |
|------|------|
| 主机名 | `LAB-Magentic-UI-02` |
| IP 地址 | `10.87.5.182/24`（静态） |
| 网关 | `10.87.5.1` |
...
| `/home` (lv-home) | 剩余 | 用户目录、Agentic-UI 项目 |
...
```powershell
# 进入 autoinstall 目录
cd c:\Users\zhlo\Documents\GIT\AI\Ubuntu-MagenticUI\LAB-Magentic-UI-02\autoinstall
...
2. 按上方"硬件配置"表分配资源
3. **CPU → 可选勾选"向客户操作系统公开硬件辅助的虚拟化"**（Expose hardware assisted virtualization）——本方案不依赖 KVM，但保留兼容性
...
> **⚠️ 关键：** 本方案使用 Playwright 直接控制浏览器，不需要嵌套虚拟化。但如需同时运行其它 VM 工作负载，建议仍开启。
...
```bash
# 上传脚本到服务器
scp harden-ubuntu.sh deploy-agentic-ui.sh magentic@<服务器IP>:~/
...
# 部署 Agentic-UI + Playwright（连接远程 Ollama）
...
- **编排器推理**：qwen3:32b 43 t/s，每次规划约 1-3 秒
- **浏览器推理**：qwen2.5vl-fast 每次视觉动作约 10-30 秒（Playwright 直接控制，无需 VM）
- **优化**：仅保留当前单页截图，无历史图片累积
- **完整浏览器任务**：5-6 个动作 × 20s + 浏览器启动 3s = 约 2 分钟
...
- **Step 7**: CIS 防火墙和网络加固（UFW: SSH + Agentic-UI 端口）
- **Step 8**: Fail2ban 暴力破解防护
- **Step 9**: 内核参数优化（Docker + Agentic-UI + CIS）
- **Step 10**: 系统限制配置（文件描述符、进程数）
- **Step 11**: Swap 配置
- **Step 12**: 日志和监控配置（Agentic-UI 日志轮转）
...
编辑 `deploy-agentic-ui.sh` 顶部配置区：

```bash
OLLAMA_HOST="http://10.87.5.55:11434"    # DGX Spark 的实际 IP
ORCHESTRATOR_MODEL="qwen3:32b"            # 编排器模型（32B，规划任务）
BROWSER_MODEL="qwen2.5vl-fast"            # 浏览器代理模型（8B，vision + JSON 动作）
AGENTIC_PORT=8081                          # Web UI 端口
```
...
| Docker 启动失败 | 加固脚本中的 `userns-remap` 或存储驱动不兼容 | 已修复：使用 `overlay2` 并移除 `userns-remap`，重新运行 `harden-ubuntu.sh` 或 `deploy-agentic-ui.sh` |
| 其它机器访问报 `Bad Host header` | FastAPI 只接受 localhost Host 头 | 已修复：脚本自动配置 nginx 反向代理，无需手动处理 |
| Ollama 连接失败 | DGX Spark 未就绪 | 确认 `OLLAMA_HOST=0.0.0.0`、防火墙 11434、网络连通 |
| Agentic-UI 启动失败 | Python 或依赖问题 | 检查 `python3.12 --version`、重新运行 `deploy-agentic-ui.sh` |
| Playwright 浏览器启动失败 | Chromium 依赖缺失 | 运行 `playwright install-deps chromium` 或重新运行 `deploy-agentic-ui.sh` |
| `/dev/kvm` 不存在 | 本方案不需要 KVM，可忽略 | 如需运行其它 VM，执行 `sudo modprobe kvm kvm_intel` 并在 ESXi 开启嵌套虚拟化 |
| `Chat completion failed after N attempts` | Ollama 推理超时 | 1) 确认模型已加载 `ollama ps` 2) 部署脚本自动设置 `num_ctx=8192/16384` 3) 检查 `journalctl -u agentic-ui -f` |
