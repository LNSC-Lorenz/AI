...
<#
.SYNOPSIS
    Create cloud-init cidata ISO for Ubuntu Autoinstall (LAB-Magentic-UI-03)
.DESCRIPTION
...
.EXAMPLE
    ...
    Create cidata03.iso in script directory with default settings
.EXAMPLE
    .\Create-CidataISO.ps1 -OutputPath "D:\VMs\cidata03.iso"
...
# Set defaults after param block (PSScriptRoot is only available after script starts)
if ([string]::IsNullOrEmpty($SourceDir)) { $SourceDir = $PSScriptRoot }
if ([string]::IsNullOrEmpty($OutputPath)) { $OutputPath = Join-Path $PSScriptRoot "cidata03.iso" }
...
Write-Host "  For Ubuntu 24.04 LTS Autoinstall (LAB-Magentic-UI-03)" -ForegroundColor Blue
Write-Host "=============================================================" -ForegroundColor Blue
    ...
    switch ($choice) {
        "1" {
            ...
            try {
                ...
                Write-Host "  Installation path: C:\Program Files (x86)\Windows Kits\10\" -ForegroundColor Gray
                
                # Install only Deployment Tools (silent mode)
                $installArgs = @(
                    "/quiet",
                    "/norestart",
...
A. Use cdrtools (mkisofs):
   ...
   Command: mkisofs -o cidata03.iso -V cidata -J -R user-data meta-data

B. Use WSL/Linux:
   sudo apt-get install genisoimage
   genisoimage -output cidata03.iso -volid cidata -joliet -rock user-data meta-data
"@ -ForegroundColor Yellow
            exit 1
        }
    }
