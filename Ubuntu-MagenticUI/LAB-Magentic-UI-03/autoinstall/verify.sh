#!/bin/bash
# Ubuntu 24.04 LTS Autoinstall 验证脚本 - Magentic-UI (LAB-03)
# 运行方式: sudo bash verify.sh

echo "=========================================="
echo "  Ubuntu Autoinstall 验证 (LAB-Magentic-UI-03)"
echo "=========================================="
echo ""
...
if [ -e /dev/kvm ]; then
    ...
    echo "/dev/kvm 不存在 ✗ (需在 ESXi 开启嵌套虚拟化)"
fi
echo ""
...
# 10. Magentic-UI venv检查（LAB-03 预装）
echo "[10/11] Magentic-UI venv（预装）"
if [ -f /home/magentic/magentic-lite/.venv/bin/magentic-ui ]; then
    echo "Magentic-UI venv 已就绪 ✓"
    su - magentic -c '/home/magentic/magentic-lite/.venv/bin/magentic-ui --version 2>/dev/null' || echo "(版本检查跳过)"
else
    echo "venv 未预装（将在 deploy 阶段创建）"
fi
echo ""

# 11. nginx检查（LAB-03 预装）
echo "[11/11] nginx（预装）"
...
echo ""
echo "后续步骤:"
echo "1. 上传并运行安全加固: sudo bash harden-ubuntu.sh"
echo "2. 上传并运行 Magentic-UI 部署: bash deploy-magentic-ui.sh"
echo ""
