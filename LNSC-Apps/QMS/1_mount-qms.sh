#!/bin/bash
# ════════════════════════════════════════════════════════════
#  挂载 QMS 受控文件共享盘到 Ubuntu 服务器
#  用法: sudo bash 1_mount-qms.sh
#  卸载: sudo umount /var/www/lnsc-apps/qms
# ════════════════════════════════════════════════════════════

# ── 配置区（按需修改）───────────────────────────────────────
SHARE='//10.86.180.4/Department/LNSC-06_QD-Quality/06_Public/QMS受控文件'
MOUNT_POINT='/var/www/lnsc-apps/qms'
CRED_FILE='/etc/qms-cred'
...
# ── 检查 root ──
if [ "$EUID" -ne 0 ]; then
  echo "请用 sudo 运行: sudo bash 1_mount-qms.sh"
  exit 1
fi
