...
- 前端 `db.js` 自动探测 API 地址：先试 `<页面目录>api/`（反代方式），不通则自动改为**浏览器直连 `http://<服务器>:3001/api/`**（后端已带 CORS 跨域头）。因此**不改 nginx 配置也能用**：只要 CTMS 后端已启动且 3001 端口对车间电脑放行即可（仅限 HTTP 内网；若站点是 HTTPS 则必须走反代方式，否则浏览器会拦截混合内容）
